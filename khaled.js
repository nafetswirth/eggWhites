/* 
* @Author: Stefan Wirth
* @Date:   2016-02-13 22:46:37
* @Last Modified by:   Stefan Wirth
* @Last Modified time: 2016-02-14 00:05:17
*/

'use strict';

var MAX_PARAGRAPH_LENGTH = 500;
var MIN_PARAGRAPH_LENGTH = 300;

module.exports.generate = function(numberOfParagraphs) {
    var ipsum = '';
    for(var i = 0; i < numberOfParagraphs; i++) {
        var paragraphLength = calculateParagraphLength();
        var paragraph = '';
        while(paragraph.length < paragraphLength) {
            paragraph += ' ';
            paragraph += majorKeys[calculateMajorKeyIndex()];
        }
        ipsum += paragraph;
        ipsum += '|';
    }
...
function calculateParagraphLength() {
    return parseInt(Math.random() * (MAX_PARAGRAPH_LENGTH - MIN_PARAGRAPH_LENGTH) + MIN_PARAGRAPH_LENGTH);
}   

function calculateMajorKeyIndex() {
    return parseInt(Math.random() * majorKeys.length);
}
...
var majorKeys = [
    ...
    'You see that bamboo behind me though, you see that bamboo? Ain\'t nothin&rsquo; like bamboo. Bless up.',
